...
export function CaseStudyPlaceholder({ visual }: { visual: CaseStudyVisual }) {
  return (
    <div className="case-study-placeholder" role="group" aria-label={`Visual pendiente: ${visual.title}`}>
      <span className="case-study-placeholder__label">Visual pendiente · {visual.type}</span>
      <strong>{visual.title}</strong>
      <span>{visual.description ?? "Agregar evidencia real cuando esté disponible."}</span>
    </div>
  );
}
...
function SectionVisual({ visual }: { visual: CaseStudyVisual }) {
  if (visual.type === "Esquema conceptual" || visual.type === "Esquema") return <ConceptDiagram title={visual.title} />;
  return <CaseStudyPlaceholder visual={visual} />;
}

function DecisionBlock({ decision }: { decision: CaseStudyDecision }) {
  return (
    <article className="case-study-decision">
      ...
      <SectionVisual visual={decision.visual} />
    </article>
  );
}

function CaseStudySection({ section }: { section: CaseStudySection }) {
  return (
    <section className="case-study-section" aria-labelledby={section.headline ? `case-section-${section.number}` : `case-section-label-${section.number}`}>
      ...
      <div className="case-study-section__body">
        ...
        {section.visual && <SectionVisual visual={section.visual} />}
        {section.blocks && (
          <div className="case-study-blocks">
            {section.blocks.map((block) => (
              <article key={block.title} className="case-study-block">
                ...
                {block.visual && <SectionVisual visual={block.visual} />}
              </article>
            ))}
          </div>
        )}
  ...
  return (
    <div className="pz-page case-study-page">
      <style>{`
        ...
        .case-study-eyebrow,.case-study-section__label,.case-study-section__number,.case-study-placeholder__label,.case-study-decision__number,.case-study-navigation__next span{font:700 10px/1.4 "Space Mono",monospace;letter-spacing:.12em;text-transform:uppercase}
        ...
        .case-study-section{display:grid;grid-template-columns:150px minmax(0,820px);gap:clamp(26px,6vw,90px);padding:54px 0;border-top:1px solid rgba(23,59,68,.2)}
        ...
        .case-study-placeholder{display:grid;gap:9px;min-height:180px;align-content:center;padding:26px;margin-top:30px;background:rgba(23,59,68,.055);border:1px dashed rgba(23,59,68,.42);border-radius:8px;color:var(--mar-profundo)}.case-study-placeholder__label{color:var(--terracota-viva)}.case-study-placeholder strong{font:600 26px/1.05 "Space Grotesk",sans-serif}.case-study-placeholder span:last-child{font:14px/1.45 "Space Grotesk",sans-serif;max-width:500px;color:rgba(23,59,68,.7)}
      ...
      <main>
        ...
        <div className="case-study-main">{study.sections.map((section) => <CaseStudySection key={section.number} section={section} />)}</div>
        {study.decisions && <section className="case-study-decisions" aria-labelledby="case-decisions-title"><h2 id="case-decisions-title">Diseñar NODUX ha sido decidir cómo deben conectarse sus partes.</h2>{study.decisions.map((decision) => <DecisionBlock key={decision.number} decision={decision} />)}</section>}
